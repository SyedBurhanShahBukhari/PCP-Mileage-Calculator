...
export function ForecastCard({ result, input }: ForecastCardProps) {
  ...
  const overage = result.projectedOverageMiles ?? 0;
    ...
    <section className="result-card result-card--forecast" aria-labelledby="result-forecast-title">
      <div className="result-card__body">
        ...
        <dl className="forecast-tiles">
          ...
          <div className={`forecast-tile${overage > 0 ? ' forecast-tile--over' : ''}`}>
            <dt>Projected excess</dt>
            <dd className="tabular" data-testid="projected-excess">
              {formatMilesWithUnit(overage)}
            </dd>
          </div>
        </dl>
        ...
        {charge ? (
          ...
          >
            ...
            <p className="charge-panel__amount" data-testid="estimated-charge">
              {formatCurrencyFromPence(charge.totalPence)}
            </p>

            {charge.vatUncertain && (
              <p className="charge-panel__note" data-testid="vat-uncertain-note">
                Shown before any additional VAT. We have not added VAT because your
                agreement&rsquo;s VAT treatment is unconfirmed.
              </p>
            )}
